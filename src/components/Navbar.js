import React, { useEffect } from "react";
import { connect, useDispatch } from "react-redux";
import { NavLink, useHistory, withRouter } from "react-router-dom";
import { compose } from "redux";
import {
  Navbar as MaterializeNavbar,
  Icon,
  NavItem,
  Dropdown,
  Divider,
} from "react-materialize";
import M from "materialize-css";
import { logoutAction } from "../store/actions/authActions";
import { round } from "lodash";
import { roundTo2Dec } from "../utils/utils";
import bettorsight_logo_light from "../static/bettorsight-logo-light.svg";

const Avatar = (props) => {
  const {
    img = "data:image/png;base64,",
  } = props;
  return (
    <div className="avatar">
      <img src={img} />
    </div>
  );
};

const UserDropDown = (props) => {
  const { userData } = props;
  const dispatch = useDispatch();
  return (
    <Dropdown
      id="avatar_dropdown"
      options={{
        alignment: "left",
        autoTrigger: false,
        closeOnClick: true,
        constrainWidth: true,
        container: null,
        coverTrigger: false,
        hover: false,
        inDuration: 150,
        onCloseEnd: null,
        onCloseStart: null,
        onOpenEnd: null,
        onOpenStart: null,
        outDuration: 250,
      }}
      trigger={<div>{React.createElement(Avatar, userData)}</div>}
    >
      <div className="card dark-card p-lr-10">
        <div className="row-flex plr-10 justify-space-between">
          <Avatar img={userData?.img} />
          <div className="white-text flex-grow">
            <p className="bold">{`${userData?.firstName || ""} ${
              userData?.lastName || ""
            }`}</p>
            <p className="bold">{`${userData?.email}`}</p>
          </div>
          <div
            onClick={() => {
              dispatch(logoutAction());
            }}
            className="white-text cursor-pointer"
          >
            <Icon>exit_to_app</Icon>
          </div>
        </div>
      </div>
    </Dropdown>
  );
};

function Navbar(props) {
  const { isAuthenticated, authuid, profile, logout } = props;
  const history = useHistory();

  var navbarElem;
  var navbraInstance;
  useEffect(() => {
    navbarElem = document.querySelector(
      ".sidenav.PrimarySideNavLinksContainer"
    );
    navbraInstance = M.Sidenav.init(navbarElem, {});
    // console.log(navbarElem);
    // console.log(navbraInstance);
  });
  const openNavbar = () => {
    navbraInstance.open();
  };
  const closeNavbar = () => {
    navbraInstance.close();
  };
  const LoginBtn = (
    <div
      onClick={() => {
        history.push("/login");
      }}
      className="btn accent1-btn m5"
    >
      Login
    </div>
  );
  const SignupBtn = (
    <div
      onClick={() => {
        history.push("/signup");
      }}
      className="btn accent2-btn m5"
    >
      Signup
    </div>
  );
  return (
    <div className="NavbarComonent">
      <div className="navbar-fixed">
        <nav>
          <div className="nav-wrapper row-flex justify-space-between">
            <div className="row-flex">
              <a onClick={openNavbar} className="cursor-pointer">
                <i className="material-icons">menu</i>
              </a>
              <div className="img-container">
                <img src={bettorsight_logo_light} />
              </div>
            </div>
            <div className="flex-spacer"></div>
            {isAuthenticated && (
              <>
                <div className="chip app-chip light-chip row-flex m5">
                  <Icon>attach_money</Icon>
                  <span className="bold">
                    {roundTo2Dec(parseFloat(profile?.credits || 0))}
                  </span>
                </div>
                <span></span>
                <NavLink to="/">
                  <div className="m5">
                    <Icon>settings</Icon>
                  </div>
                </NavLink>
                <UserDropDown userData={{ ...profile }} />
              </>
            )}
            {!isAuthenticated && (
              <>
                {LoginBtn}
                {SignupBtn}
              </>
            )}
          </div>
        </nav>
      </div>
      <ul className="sidenav PrimarySideNavLinksContainer" id="mobile-demo">
        <li>
          <NavLink to="/">
            <Icon>home</Icon>
            <span>Home</span>
          </NavLink>
        </li>

        {isAuthenticated && (
          <>
            <li>
              <NavLink to="/deposits">
                <Icon>account_balance_wallet</Icon>
                <span>Deposits</span>
              </NavLink>
            </li>
            <li>
              <NavLink to="/withdrawals">
                <Icon>account_balance</Icon>
                <span>Withdrawals</span>
              </NavLink>
            </li>
            <li>
              <NavLink to="/">
                <Icon>games</Icon>
                <span>Games</span>
              </NavLink>
            </li>
            <li>
              <NavLink to="/history">
                <Icon>history</Icon>
                <span>History</span>
              </NavLink>
            </li>
            <li>
              <NavLink to="/provablyfair">
                <Icon>address</Icon>
                <span>Provably Fair</span>
              </NavLink>
            </li>
            <li>
              <NavLink to="/">
                <Icon>star</Icon>
                <span>Leaderboard</span>
              </NavLink>
            </li>
          </>
        )}
        {!isAuthenticated && (
          <>
            {LoginBtn}
            {SignupBtn}
          </>
        )}
      </ul>
    </div>
  );
}

const mapStateToProps = (state) => {
  return {
    authuid: state.firebase.auth.uid,
    isAuthenticated: (state.firebase.auth.uid && true) || false,
    profile: state.firebase.profile,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    logout: () => {
      dispatch(logoutAction());
    },
  };
};

export default compose(
  connect(mapStateToProps, mapDispatchToProps),
  withRouter
)(Navbar);
